...
Comment: during the first pass, we need to first collect all labels first, since it is possible that some of variables are labels 
*/
...
int main(int argc, char const *argv[]){
    ...
    for(i = 0; i < 2; i++){
        ...
        while (fgets(str, MAXCHAR, inFp) != NULL){
            ...
            DPRINTF("i = %d, post-processed str = %s\n",i,str);
            ...
            if(*iter == '@'){ //A instruction
                ...
                else{ //for pass 2, replace symbols (here can be either variable name or label name) with allocated memory address
                    ...
                    while(*iter){
                        if(*iter == ')'){
                            ...
                            if(!linked_list_check_node_avail((const char*)symbolName, symbol_table)){
                                ...
                                linked_list_change_node_address(symbol_table, (const char*)symbolName, lineCnt+1);
                                DPRINTF("line number associated with %s = %d\n",symbolName,lineCnt+1);
                            }
                        }
                        iter++;
                    }
